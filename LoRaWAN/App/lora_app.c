...
/* External variables ---------------------------------------------------------*/
/* USER CODE BEGIN EV */
extern volatile int Activity_detected;
extern uint8_t currentPositionx;
extern uint8_t currentPositiony;
...
#define REG_THRESHOLD_TAP 0X1D
#define REG_DURATION_TAP  0X21  // READ/WRITE
#define REG_DEVICE_ID     0X00  //READ ONLY
#define REG_INT_MAP	      0X2F //READ /WRITE
#define REG_INT_EN	      0X2E //READ /WRITE
#define REG_INT_SOURCE    0X30 //READ
#define ADXL_DDEVICE_ID	  0xE5  //READ ONLY
/* USER CODE END PM */
...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params)
{
  ...
  if (params != NULL)
  {
    HAL_GPIO_WritePin(LED3_GPIO_Port, LED3_Pin, GPIO_PIN_SET); /* LED_BLUE */
...
static void SendTxData(void)
{
    ...
    if (!LmHandlerIsBusy())
    {
        ...
        // Log current position x and y
//        APP_LOG(TS_ON, VLEVEL_M, "current position x: %d\r\n", positionx);
//        APP_LOG(TS_ON, VLEVEL_M, "current position y: %d\r\n", positiony);

        AppData.Port = LORAWAN_USER_APP_PORT;

        // Prepare the payload, ensure endianness matches network expectations
        uint32_t i = 0;
//        int32_t roundedX = currentPositionx;
//        int32_t roundedY = currentPositiony;
        APP_LOG(TS_ON, VLEVEL_M, "current position x: %d\r\n", currentPositionx);
            APP_LOG(TS_ON, VLEVEL_M, "current position y: %d\r\n", currentPositiony);

        // Encode roundedX as bytes
            uint8_t *px = (uint8_t *)&currentPositionx;
               AppData.Buffer[i++] = px[3];
               AppData.Buffer[i++] = px[2];
               AppData.Buffer[i++] = px[1];
               AppData.Buffer[i++] = px[0];

               // Encode currentPositiony as bytes
               uint8_t *py = (uint8_t *)&currentPositiony;
               AppData.Buffer[i++] = py[3];
               AppData.Buffer[i++] = py[2];
               AppData.Buffer[i++] = py[1];
               AppData.Buffer[i++] = py[0];
...
static void StopJoin(void)
{
  ...
  HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, GPIO_PIN_SET); /* LED_BLUE */
  HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, GPIO_PIN_SET); /* LED_GREEN */
  HAL_GPIO_WritePin(LED3_GPIO_Port, LED3_Pin, GPIO_PIN_SET); /* LED_RED */
...
static void OnStopJoinTimerEvent(void *context)
{
  ...
  HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, GPIO_PIN_RESET); /* LED_BLUE */
  HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, GPIO_PIN_RESET); /* LED_GREEN */
  HAL_GPIO_WritePin(LED3_GPIO_Port, LED3_Pin, GPIO_PIN_RESET); /* LED_RED */
